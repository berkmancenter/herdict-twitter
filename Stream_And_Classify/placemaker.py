...
class Place(object):
    ...
    def __init__(self, tree):
        self.place = tree.find('{}place'.format(TAG_PREFIX))
        
        self.woeid = self.place.find('{}woeId'.format(TAG_PREFIX)).text
        try:
            self.woeid = int(self.woeid)
        except ValueError:
            pass
        
        self.placetype = self.place.find('{}type'.format(TAG_PREFIX)).text
        self.name = self.place.find('{}name'.format(TAG_PREFIX)).text
        self.name = self.name.encode('utf-8', 'ignore')
        
        self.centroid = PlacemakerPoint(self.place.find('{}centroid'.format(TAG_PREFIX)))
        
        self.match_type = tree.find('{}matchType'.format(TAG_PREFIX)).text
        try:
            self.match_type = int(self.match_type)
        except ValueError:
            pass
        
        self.weight = tree.find('{}weight'.format(TAG_PREFIX)).text
        try:
            self.weight = int(self.weight)
        except ValueError:
            pass
        
        self.confidence = tree.find('{}confidence'.format(TAG_PREFIX)).text
        try:
    ...
    def __repr__(self):
        return "<Placemaker Place: '{}'>".format(self.name)


class Scope(object):
    
    def __init__(self, tree):
        self.name = tree.find('{}name'.format(TAG_PREFIX)).text
        if self.name:
            self.name = self.name.encode('utf-8', 'ignore')
        
        self.woeid = tree.find('{}woeId'.format(TAG_PREFIX)).text
        try:
            self.woeid = int(self.woeid)
        except ValueError:
            pass
        
        self.placetype = tree.find('{}type'.format(TAG_PREFIX)).text
        self.centroid = PlacemakerPoint(tree.find('{}centroid'.format(TAG_PREFIX)))
    
    def __repr__(self):
        return u"<Placemaker Scope: '{}'>".format(self.name)


class AdministrativeScope(Scope):
    
    def __repr__(self):
        return u"<Placemaker AdministrativeScope: '{}'>".format(self.name)


class GeographicScope(Scope):
    
    def __repr__(self):
        return u"<Placemaker GeographicScope: '{}'>".format(self.name)


class PlacemakerPoint(object):
    
    def __init__(self, tree):
        self.latitude = tree.find('{}latitude'.format(TAG_PREFIX)).text
        if self.latitude:
            self.latitude = float(self.latitude)
        
        self.longitude = tree.find('{}longitude'.format(TAG_PREFIX)).text
        if self.longitude:
            self.longitude = float(self.longitude)
    
    def __repr__(self):
        return u"<Placemaker Point: '{}, {}'>".format(self.latitude, self.longitude)


class Extents(object):
    
    def __init__(self, tree):
        self.center = PlacemakerPoint(tree.find('{}center'.format(TAG_PREFIX)))
        self.southwest = PlacemakerPoint(tree.find('{}southwest'.format(TAG_PREFIX)))
        self.northeast = PlacemakerPoint(tree.find('{}northeast'.format(TAG_PREFIX)))
...
class placemaker(object):
    ...
    def find_places(self, input, documentType='text/plain', inputLanguage='en-US', 
        ...
        self.doc = self.tree.find('{}document'.format(TAG_PREFIX))
        
        administrative_scope_tree = self.doc.find('{}administrativeScope'.format(TAG_PREFIX))
        if administrative_scope_tree is not None:
            self.administrative_scope = AdministrativeScope(administrative_scope_tree)
        
        geographic_scope_tree = self.doc.find('{}geographicScope'.format(TAG_PREFIX))
        if geographic_scope_tree is not None:
            self.geographic_scope = GeographicScope(geographic_scope_tree)
        
        place_details = self.doc.findall('{}placeDetails'.format(TAG_PREFIX))
